...
internal class MPVView(context: Context, attrs: AttributeSet) : BaseMPVView(context, attrs) {
    override fun initOptions() {
        ...
        // === AnimeStream: Apply user-selected shader preset ===
        // Preset key stored in shared preferences; resolved against bundled shader files.
        val shaderPresetKey = sharedPreferences.getString("shader_preset", "anime4k_a") ?: "anime4k_a"
        val preset = ShaderPresets.Preset.fromKey(shaderPresetKey)
        ShaderPresets.apply(context, preset)

        // === AnimeStream: Keep aspect ratio locked ===
        // User requirement: upscaled video must maintain original aspect ratio.
    ...
    override fun observeProperties() {
        ...
        val p = arrayOf(
            ...
            // === AnimeStream: properties for Stress Bar benchmark ===
            Property("estimated-vf-fps", MPV_FORMAT_DOUBLE),
            Property("frame-drop-count", MPV_FORMAT_INT64),
            Property("vo-delayed-frame-count", MPV_FORMAT_INT64),
