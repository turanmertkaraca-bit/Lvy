...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    // AnimeStream: coroutine scope for async YouTube URL resolution
    ...
    override fun onCreate(icicle: Bundle?) {
        ...
        // === AnimeStream: YouTube stream resolution ===
        // If the shared/viewed URL is YouTube, we need to extract the actual
        // direct stream URLs (one for video, one for audio) using NewPipeExtractor,
        // then pass them to mpv. mpv will sync them internally via audio-file.
        if (YouTubeResolver.isYouTubeUrl(filepath)) {
            val prefs = getDefaultSharedPreferences(this)
            val baseResolution = prefs.getString("youtube_base_resolution", "480p") ?: "480p"
            Log.i(TAG, "YouTube URL detected, resolving at base resolution: $baseResolution")
            showToast(getString(R.string.yt_resolving))
            playerScope.launch {
                val resolved = YouTubeResolver.resolve(filepath, baseResolution)
                withContext(Dispatchers.Main) {
                    if (resolved == null) {
                        showToast(getString(R.string.yt_resolve_failed))
                        finishWithResult(RESULT_CANCELED)
                        return@withContext
                    }
                    Log.i(TAG, "YouTube resolved: ${resolved.resolution} → ${resolved.videoUrl.take(80)}")
                    // Attach audio as external track — mpv handles A/V sync internally
                    if (resolved.audioUrl != null) {
                        MPVLib.setOptionString("audio-file", resolved.audioUrl)
                    }
                    player.playFile(resolved.videoUrl)
                }
            }
        } else {
            // Normal file/URL — play directly
    ...
    override fun onNewIntent(intent: Intent?) {
        ...
        if (!activityIsForeground && didResumeBackgroundPlayback) {
            // === AnimeStream: YouTube resolution for background loadfile ===
            if (YouTubeResolver.isYouTubeUrl(filepath)) {
                val prefs = getDefaultSharedPreferences(this)
                val baseResolution = prefs.getString("youtube_base_resolution", "480p") ?: "480p"
                showToast(getString(R.string.yt_resolving))
                playerScope.launch {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        val mode = if (this@MPVActivity.newIntentReplace) "replace" else "append"
                        MPVLib.command(arrayOf("loadfile", resolved.videoUrl, mode))
                    }
                }
                moveTaskToBack(true)
            } else if (this.newIntentReplace) {
            ...
            // === AnimeStream: YouTube resolution for foreground loadfile ===
            if (YouTubeResolver.isYouTubeUrl(filepath)) {
                val prefs = getDefaultSharedPreferences(this)
                val baseResolution = prefs.getString("youtube_base_resolution", "480p") ?: "480p"
                showToast(getString(R.string.yt_resolving))
                playerScope.launch {
                    ...
                    withContext(Dispatchers.Main) {
                        ...
                        MPVLib.command(arrayOf("loadfile", resolved.videoUrl))
                    }
                }
            } else {
